...
import {
    getUniqueId, jsonify, JsonRpcError, Maybe, SafeEventEmitter, serializeError,
} from './utils'
...
export class JsonRpcEngine extends SafeEventEmitter {
    ...
    handle<T, U>(requests: JsonRpcRequest<T>[], callback: (error: unknown, responses: JsonRpcResponse<U>[]) => void): void; // eslint-disable-line max-len
...
export const createMetaRPCHandler = <T extends {}, S extends Duplex>(
    api: T,
    outStream: S,
