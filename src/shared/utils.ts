...
export const currentUtime = (clockOffset: number) => ((new Date().getTime() + clockOffset) / 1000) | 0;
...
export const getRpcPromiseCallback = (resolve: (value?: any) => void, reject: (error?: Error) => void, unwrapResult = true) => (error: Error, response: PendingJsonRpcResponse<unknown>) => {
  if (error || response.error) {
    ...
    if (!unwrapResult || Array.isArray(response)) {
      resolve(response);
    } else {
...
export const extractTransactionAddress = (transaction: Transaction): { direction: TransactionDirection; address: string } => {
  const transactionWithInfo = transaction as TonWalletTransaction;
  if (
    transactionWithInfo.info?.type === 'wallet_interaction' &&
    transactionWithInfo.info.data.method.type === 'multisig'
...
export const parseCurrency = (amount: string, decimals: number) => new Decimal(amount).mul(multiplier(decimals)).ceil().toFixed(0);
