...
import { StandaloneConnectionController } from './StandaloneConnectionController';
...
export class StandaloneController extends EventEmitter {
  ...
  public static async load(options: StandaloneControllerOptions) {
    const { nekoton, origin, jrpcClient, getDomainMetadata } = options;

    const counters = new Counters();
    const clock = new nekoton.ClockWithOffset();

    const connectionController = new StandaloneConnectionController({
      clock,
    ...
    const subscriptionsController = new StandaloneSubscriptionController({
      clock,
      connectionController: connectionController as any as ConnectionController, // TODO: remove
    });
  ...
  public getApi() {
    ...
    return {
      getState: (cb: ApiCallback<ReturnType<typeof StandaloneController.prototype.getState>>) => cb(null, this.getState()),
