...
import { AccountabilityStore, RpcStore, TokensManifestItem, TokensStore } from '@app/popup/modules/shared';
...
export class ApproveAddAssetViewModel implements Disposable {
  ...
  constructor(
    ...
    this.disposer = autorun(() => {
      const tokenWallet = this.approval.requestData.details.tokenWallet;

      this.rpcStore.rpc.getTokenWalletBalance(tokenWallet)
        .then(action((balance) => { this.balance = balance; }))
        .catch(action(() => { this.balance = '0'; }));
    });
  }
  ...
  get phishingAttempt(): PhishingAttempt | undefined {
    ...
    if (this.tokensMeta) {
      for (const { rootTokenContract } of additionalAssets) {
        ...
        existingToken = this.tokensMeta[info.rootTokenContract] ? ExistingToken.Trusted : ExistingToken.Untrusted;
        break;
      }
    }
